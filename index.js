...
const prohashing = function (config) {
	...
	self.connectionOpen = (session, details) => {
		...
		if (self.subscribe.indexOf('profitability') > -1 || self.subscribe == "all") wampSession.call('f_all_miner_updates', [self.config.apiKey]).then(self.initialProfitabilityUpdatesReceived)
